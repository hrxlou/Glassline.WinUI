...
| AppleReferenceLab skeleton | **Done for M0 buildable-probe task** | SwiftUI/AppKit probe tests + release build pass on macOS 26 CI; 43 of 84 required captures held at a measured 2.0000 px/pt; 17 inactive-window captures carry an active-window header and must be retaken, and 24 interaction captures are newly required |
...
- **2026-08-14:** Ran the titlebar material spike on native Windows. The WinUI `TitleBar` control composes inside `GlasslineGlassContainer` with the band resolving to Full material and system caption glyphs still legible over it, so Glassline does not need a bespoke titlebar primitive. Measured `RightInset=138`, `LeftInset=0`, `Height=32` — and those values read as 0 unless the app has opted into owning the titlebar, which makes ADR-0011's query-do-not-assume rule the only way to obtain them. ADR-0012 on band composition is unblocked.
- **2026-08-14:** Added the interaction axis — `hover`, `pressed`, `focused` on a designated state probe in four scenes — so state clarity, which the evidence boundary classes as observable, stops being unobserved. The required set is 84. Added a guided capture session script so the operator never types a filename, which is where every defect in the first three deliveries lived.
- **2026-08-14:** Third macOS session completed the contrast variant and delivered all 60 required filenames at a measured 2.0000 px/pt. Verification then found that 17 of the 18 inactive-window captures carry a header reading `window=active`; only `buttons__dark__inactive__default` is genuine. Two earlier reviews of the same set missed this, because filename and calibration checks cannot detect it. `verify-captures.py` now compares the in-capture headers, and the first six ledger rows were corrected from six captures to four or five distinct captured states. The measured values were unchanged; only the claimed evidence strength was wrong. Inactive-window material response is effectively unobserved.
- **2026-08-14:** Second macOS 26.5.2 session delivered 54 of 72 required captures at a calibration-measured 2.0000 px/pt with zero deviation across the set, every filename matching its `capture_id`, plus 4 open-menu/popover observations. The 18 `increase-contrast` captures were blocked by a matrix defect rather than by the hardware: macOS couples Increase Contrast to Reduce Transparency, so the isolated contrast variant the matrix demanded does not exist. The variant now means the coupled state macOS exposes. No ledger rows added yet.
