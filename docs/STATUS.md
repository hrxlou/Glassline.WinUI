...
| AppleReferenceLab skeleton | **Done for M0 buildable-probe task** | SwiftUI/AppKit probe tests + release build pass on macOS 26 CI; all 60 required filenames delivered at a measured 2.0000 px/pt, but only 43 hold a distinct captured state — 17 inactive-window captures carry an active-window header and must be retaken |
